...
def get_loss(loss_config: LossConfig) -> 'Loss':
    ...
    elif loss_config.name == C.CROSS_ENTROPY_ALIGNMENT:
        return CrossEntropyAlignmentLoss(loss_config)
    else:
...
class AlignmentCost:
    """
    Computes alignment cost as in Chen et al. 2016
    """
    def __init__(self):
        pass
    def get_loss(self,  attn_probs, align_labels) -> mx.sym.Symbol:
        """
        :param align_labels: Shape: (batch_size, target_seq_len, input_seq_len).
        :param attn_probs: Shape: (batch_size, target_seq_len, input_seq_len).
        """
        log_mat = mx.sym.log(attn_probs)
        prod_mat = align_labels * log_mat
        return mx.sym.MakeLoss(-mx.sym.mean(prod_mat, axis=(0)))
    
class CrossEntropyAlignmentLoss(Loss):
    """
    Computes cross-entropy loss between alignment and reference and between target and target reference

    :param loss_config: Loss configuration
    """

    def __init__(self, loss_config: LossConfig) -> None:
        logger.info("Loss: AlignmentLoss(normalization_type=%s, label_smoothing=%s)",
                     loss_config.normalization_type, loss_config.label_smoothing)
        self.loss_config = loss_config

    def get_loss(self, 
                decoder_logits: mx.sym.Symbol, 
                decoder_labels: mx.sym.Symbol,    
                align_probs: mx.sym.Symbol,
                align_labels: mx.sym.Symbol) -> List[mx.sym.Symbol]:
        """
        Returns alignment and translation loss and softmax output symbols given logits and integer-coded labels
        for decoder outputs and alignment loss computed according to Chen et al. 2016. 

        :param decoder_logits: Shape: (batch_size * target_seq_len, target_vocab_size).
        :param decoder_labels: Shape: (batch_size * target_seq_len,).
        :param align_probs: Shape: (batch_size, target_seq_len, input_seq_len).
        :param align_labels: Shape: (batch_size, target_seq_len, input_seq_len). 
        ...
        return [mx.sym.SoftmaxOutput(data=decoder_logits,
                                     label=decoder_labels,
                                     ignore_label=C.PAD_ID,
                                     use_ignore=True,
                                     normalization=normalization,
                                     smooth_alpha=self.loss_config.label_smoothing,
                                     name=C.SOFTMAX_NAME),
                AlignmentCost().get_loss(align_probs, align_labels)]

    def create_metric(self) -> "CrossEntropyAlignmentMetric":
        #TODO: create a new metric for this loss 
