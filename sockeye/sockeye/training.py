...
class TrainingModel(model.SockeyeModel):
    ...
    def _initialize(self,
        ...
        def sym_gen(seq_lens):
            ...
            target_decoded, all_probs  = self.decoder.decode_sequence(source_encoded, source_encoded_length, source_encoded_seq_len,
                                                          target_embed, target_embed_length, target_embed_seq_len)
            all_probs_mx = mx.sym.stack(*all_probs, axis=0)
            #(n_layers, batch, heads, source_length)
            if self.config.attention_mode == -1:
                # do average
                logger.info([type(x) for x in all_probs])
                alignment_probs = mx.sym.mean(all_probs_mx, axis=(0,  2), keepdims=False)
            #else:
                # otherwise get alignment at single attention layer
           # alignment_probs = all_probs[self.config.attention_mode]
            #alignment_probs = all_probs[0]
            ...
            #TODO: this may be wrong, if there's an issue check here first
            align_labels_reshaped = mx.sym.reshape(data=mx.sym.Variable(C.ALIGNMENT_LABEL_NAME), shape=(-1, target_seq_len, source_seq_len ))
            #@Matt this is where a reshaping error gets thrown  
            alignment_probs_reshaped = mx.sym.reshape(alignment_probs, shape=(-1, target_seq_len, source_seq_len ))

            logits = self.output_layer(target_decoded)

            loss_output = self.model_loss.get_loss(logits, labels, alignment_probs_reshaped, align_labels_reshaped)
        ...
        if self._bucketing:
            ...
            self.module = mx.mod.Module(symbol=symbol,
                                        ...
                                        fixed_param_names=fixed_param_names)
        logger.info("module:")
        logger.info(self.module.__dict__)
        logger.info("data shapes:")
        logger.info(provide_data)
        logger.info("label shapes: ")
