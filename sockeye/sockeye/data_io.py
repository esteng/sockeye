...
class RawAlignmentDatasetLoader:
    """
    Loads a data set of variable-length parallel source/target sequences along with alignments into buckets of NDArrays.
    ...
    def make_alignment_matrix(self, a, source_len, target_len):
        # alignment matrices are target x source
        alignment_data = np.zeros((target_len , source_len ))
        normalizer = np.zeros((target_len))
        for (src_idx, tgt_idx) in a:
            alignment_data[tgt_idx, src_idx] = 1
            normalizer[tgt_idx] += 1
        normalizer[normalizer == 0] = 1
        normalizer = normalizer.reshape(-1,1)
        # normalize
        return alignment_data/normalizer

    def load(self,
             source_iterables: Sequence[Iterable],
             alignment_iterable: Iterable,
        ...
        # define alignment truth matrices for each bucket
        # need to add one since 0 reserved for BOS symbol
        data_alignment = [np.zeros((num_samples, target_len , source_len ))  
                        for (source_len, target_len), num_samples in zip(self.buckets, num_samples_per_bucket)]
        ...
        for sources, alignment, target in triple_iter(source_iterables, alignment_iterable, target_iterable):
            source_len = len(sources[0])
            target_len = len(target)
            ...
            #logger.info("source shape is {}".format(data_source[buck_index][sample_index,:,:].shape))
            #logger.info("alignment: {}".format(alignment))
            #logger.info("source length: {}, target length: {}".format(source_len, target_len))
            #logger.info("source is : {}".format(list(sources)))
            ref_shape = data_alignment[buck_index][sample_index,:,:].shape
            try:
                arr_to_fill = self.make_alignment_matrix(alignment, ref_shape[1], ref_shape[0])
            except IndexError:
                logger.info("source was: {}".format(sources))
                logger.info("target was: {}".format(target))
                sys.exit()
            data_alignment[buck_index][sample_index] += arr_to_fill
        ...
        for i in range(len(data_source)):
            ...
            #try:
            data_alignment[i] = mx.nd.array(data_alignment[i], dtype=self.dtype)
            #except TypeError:
            #    print(data_alignment[i])
            #    sys.exit()
        ...
        return ParallelDataSet(data_source,  data_target, data_label, data_alignment)
...
def get_validation_data_iter(data_loader: RawParallelDatasetLoader,
                             ...
                             batch_size: int,
                             fill_up: str,
                             validation_alignment_source: Optional[List[str]] = None) -> 'ParallelSampleIter':
    """
    ...
    validation_length_statistics = analyze_sequence_lengths(validation_sources, validation_target,
                                                            source_vocabs, target_vocab,
                                                            max_seq_len_source, max_seq_len_target)
    if validation_alignment_source is not None:
        validation_sources_sentences, validation_target_sentences, validation_alignment_sentences = create_sequence_readers(validation_sources,
                                                                                        validation_target,
                                                                                        source_vocabs, 
                                                                                        target_vocab,
                                                                                        validation_alignment_source)
    else:
    ...
    if validation_alignment_source is not None:
        validation_data = data_loader.load(validation_sources_sentences,validation_alignment_sentences , validation_target_sentences, 
...
def get_training_data_iters(sources: List[str],
                            ...
                            validation_alignment_source: str = None) -> Tuple['BaseParallelSampleIter',
    ...
    :param alignment_source: Path to alignment training data
    ...
    if alignment_source is not None:
        sources_sentences, target_sentences, alignment_sequences = create_sequence_readers(sources, target, source_vocabs, target_vocab, alignment_source)
    else:
    ...
    if alignment_source is not None:
        ...
        data_loader = RawAlignmentDatasetLoader(buckets=buckets,
                                               eos_id=target_vocab[C.EOS_SYMBOL],
                                               pad_id=C.PAD_ID)
        training_data = data_loader.load(sources_sentences, alignment_sequences,target_sentences,  
    ...
    if validation_sources is not None and validation_target is not None and validation_alignment_source is not None:
        validation_iter = get_validation_data_iter(data_loader=data_loader,
                                                   validation_sources=validation_sources,
                                                   validation_target=validation_target,
                                                   buckets=buckets,
                                                   ...
                                                   batch_size=batch_size,
                                                   fill_up=fill_up, 
                                                   validation_alignment_source=validation_alignment_source)
    elif validation_sources is not None and validation_target is not None:
        validation_iter = get_validation_data_iter(data_loader=data_loader,
                                                   validation_sources=validation_sources,
                                                   validation_target=validation_target,
                                                   buckets=buckets,
...
class AlignmentReader(Iterable):
    """
    Reads a sequence of alignments from data 
    ...
    def __iter__(self):
        for tokens in read_content(self.path, self.limit):
            sequence = self.split_tokens(tokens)
            if len(sequence) == 0:
                yield None
                continue
            if self.add_bos:
            ...
            if self.add_eos:
                max_src, max_tgt = self.get_max(sequence)
                sequence.append([max_src,max_tgt])
            yield sequence

    def split_tokens(self, tokens):
        split_seq = [list(x.split("-")) for x in tokens]
        int_seq = [[int(x) for x in a] for a in split_seq]
        return int_seq
    
    def get_max(self, sequence):
        src_idxs, tgt_idxs = zip(*sequence)
        return max(src_idxs), max(tgt_idxs)

def create_sequence_readers(sources: List[str], target: str,
                            vocab_sources: List[vocab.Vocab],
                            vocab_target: vocab.Vocab,
                            alignment_source: str = None) -> Tuple[List[SequenceReader], SequenceReader]:
    """
    ...
    if alignment_source is not None:
        alignment_sequence_reader = AlignmentReader(alignment_source) 
        return source_sequence_readers, target_sequence_reader, alignment_sequence_reader
    return source_sequence_readers, target_sequence_reader


def triple_iter(source_iters: Sequence[Iterable[Optional[Any]]], alignment_iters: Iterable[Optional[Any]], target_iterable: Iterable[Optional[Any]]):
    """
    Yields parallel source(s),alignments,  target sequences from iterables.
    ...
    for sources, alignment, target in zip(zip(*source_iters), alignment_iters, target_iter):
        if any((s is None for s in sources)) or any((a is None for a in alignment)) or target is None:
            num_skipped += 1
            continue
        check_condition(are_token_parallel(sources), "Source sequences are not token-parallel: %s" % (str(sources)))
        yield sources, alignment, target
...
class ParallelDataSet(Sized):
    ...
    def __init__(self,
                 ...
                 alignment: List[mx.nd.array] = None) -> None:
    ...
    def save(self, fname: str):
        ...
        if alignment is not None:
            mx.nd.save(fname, self.source + self.target + self.label + self.alignment)
        else:
            mx.nd.save(fname, self.source + self.target + self.label)

    @staticmethod
    def load(fname: str, align:bool = False) -> 'ParallelDataSet':
        """
        ...
        if not align:
            ...
            alignment = data[3*n:]
            assert len(source) == len(target) == len(label)
            return ParallelDataSet(source, target, label, alignment)
    ...
    def fill_up(self,
        ...
        for bucket_idx in range(len(self)):
            ...
            if self.alignment is not None:
                bucket_alignment = self.alignment[bucket_idx]
            ...
            if num_samples % bucket_batch_size != 0:
                ...
                if self.alignment is not None:
                    logger.info("Concatenating alignments at {}".format(desired_indices))
                    alignment[bucket_idx] = mx.nd.concat(bucket_alignment, bucket_alignment.take(desired_indices), dim=0)
                if policy == C.FILL_UP_ZEROS:
...
class BaseParallelSampleIter(mx.io.DataIter):
    ...
    def __init__(self,
                 ...
                 label_name: str,
                 alignment_data_name: str = None,
                 alignment_label_name: str = None,
                 num_factors: int = 1,
                 permute: bool = True,
                 dtype='float32') -> None:
        ...
        self.alignment_label_name = alignment_label_name,
        ...
        if alignment_label_name is None:
            self.provide_data = [
            ...
            self.provide_data = [
                ...
                mx.io.DataDesc(name=self.target_data_name,
                               ...
                               layout=C.BATCH_MAJOR)]
            self.provide_label = [
                mx.io.DataDesc(name=self.label_name[0],
                               shape=(self.bucket_batch_sizes[-1].batch_size, self.default_bucket_key[1]),
                               layout=C.BATCH_MAJOR),
                # alignment labels are (batch, target, source)
                mx.io.DataDesc(name=self.alignment_label_name[0],
                              shape=(self.bucket_batch_sizes[-1].batch_size, self.default_bucket_key[1], self.default_bucket_key[0]),
                                layout=C.BATCH_MAJOR)]

            self.data_names = [self.source_data_name, self.target_data_name]
            self.label_names = [self.label_name, self.alignment_label_name]
...
class ParallelSampleIter(BaseParallelSampleIter):
    ...
    def __init__(self,
                 ...
                 target_data_name=C.TARGET_NAME,
                 label_name=C.TARGET_LABEL_NAME,
                 alignment_label_name = C.ALIGNMENT_LABEL_NAME,
                 num_factors: int = 1,
                 permute: bool = True,
        ...
        super().__init__(buckets=buckets, batch_size=batch_size, bucket_batch_sizes=bucket_batch_sizes,
                         source_data_name=source_data_name, target_data_name=target_data_name, 
                         label_name=label_name, alignment_label_name=alignment_label_name, num_factors=num_factors, permute=permute, dtype=dtype)
        self.with_alignment = with_alignment
        # create independent lists to be shuffled
        if self.with_alignment:
            self.data = ParallelDataSet(list(data.source), list(data.target), list(data.label), list(data.alignment))
        else: 
    ...
    def next(self) -> mx.io.DataBatch:
        ...
        if self.with_alignment:
            data = [source, target]
            alignment = self.data.alignment[i][j:j+batch_size] 
            lab = self.data.label[i][j:j + batch_size]
            label = [lab, alignment]
        else:
            data = [source, target]
        ...
        self.label_names = [x[0] for x in self.label_names]
        ...
        logger.info("PROVIDE LABEL:")
        logger.info(provide_label)
